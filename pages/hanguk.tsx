...
function Hanguk({ subtitleData, subtitleError }: { subtitleData: any; subtitleError: string }) {
  ...
  const [ccData, setCcData] = useState<MoeviewAmusementData | null>(null);
  const [descriptionData, setDescriptionData] = useState<MoeviewAmusementData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let response = await fetch('/api/hanguk?page=1&pageSize=7&hangukName=dubbing');
        if (!response.ok) {
